...
由于 QQ 风控机制的加强，go-cqhttp 若出现 Code45[^3] 报错情况时，请参照以下步骤配置签名服务器：

6. 安装 JRE 17（Java Runtime Environment 17）[^4]，请善用搜索引擎查找安装方法。

[^4]: 你也可以使用 JDK 17（Java Development Kit 17）。
    ...
    account: # 账号相关
      ...
      sign-servers:
        - url: 'http://127.0.0.1:8080'  # 主签名服务器地址， 必填
          key: '114514'  # 签名服务器所需要的 apikey, 如果签名服务器的版本在 1.1.0 及以下则此项无效
