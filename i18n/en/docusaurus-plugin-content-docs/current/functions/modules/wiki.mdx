...
-   Delete request header：`~wiki headers remove <request header>`
...
Separate lines from different headers.
...
In the face of recent attacks, we found that the grey machine had used “partially sealed sites” measures to some wiki by restricting the web site to “dog heads” APP users only.Small can be affected but can be resolved by **\*US-Agent\`** of the pseudo-dog APP.Please run the following command：
...
This command is used to set the auto-add prefix when using the `wiki` command.Once set, each query wiki will be asked with the previous prefix query.
