...
`base_superuser =` - Set bot main superuser.`base_superuser =` - Set bot main superuser.Available formats are `QQ|<QQ Account>`, `Discord|Client|<ClientID>`, `Telegram|Client|<ClientID>`, `Kook|User|<UserID>`. You can use the '~whoami' command to view your ID after the bot is started, and the object will be automatically marked as a super user after the bot is started.
