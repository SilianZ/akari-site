...
由于目前配置文件后缀改为 `toml`，与 `cfg` 不同的是，请在填写好必要的字段后，请删除所有配置文件中留空的字段，否则程序无法正常运行。Since the current configuration suffix is changed to `toml`, unlike `cfg`, please delete the empty fields in all configuration files after filling out the required fields otherwise the program cannot function properly.If you have an old `cfg` file, the robot will automatically help you convert to the `toml` format.若您拥有旧版 `cfg` 文件，机器人会自动帮您转换为 `toml` 格式。
...
2.  Unpack downloaded files into a pre-prepared folder.
    ...
    Please enter \`tar -xzvf [File name] in the command line.
...
3.  Run go-cqhttp:
    ...
    1.  Double click 'go-cqhttp\_\*.exe ', and generate and run scripts according to the prompts.
    ...
    2.  `cd` to extract directory.
    3.  Enter the `./go-cqhttp`, and `Enter` to run.
...
1.  This service is written using JavaScript, driven by `Puppeteer`, for which you need to install [Node.js](https://nodejs.org/) and [Chrome](https://google.cn/intl/en-CN/chrome/).
...
The `maimai` module is based on [mai-bot](https://github.com/Diving-Fish/mai-bot).This module requires additional resource files to work properly.此模块需要额外的资源文件才可正常工作。此模块需要额外的资源文件才可正常工作。
