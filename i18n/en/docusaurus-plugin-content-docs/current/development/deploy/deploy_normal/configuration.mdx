...
6. 安装 JRE 17（Jave Runtime Environment 17），请善用搜索引擎查找安装方法。

此服务使用 JavaScript 编写，由 `Puppeteer` 驱动，为此，您需要安装好 [Node.js](https://nodejs.org/) 环境，以及安装好 [Chrome](https://www.google.cn/intl/zh-CN/chrome/)。

7. 在 ~~[unidbg-fetch-qsign](https://github.com/fuqiuluo/unidbg-fetch-qsign)~~（作者已删库，请自行在 GitHub 上搜索有关 `qsign` 的仓库）的 Release 界面中下载最新版本的 unidbg-fetch-qsign 并解压到一个提前准备好的文件夹中。

8. 删除与 go-cqhttp 同一目录下的 `data` 文件夹和 `device.json` 文件。
...
请参照 [Webrender 文档](/docs/development/webrender#部署) 部署 Webrender。
